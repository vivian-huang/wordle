...
class WordleGame:
    def __init__(self, difficulty, player_mode):
...
def manual(width, height, main_fps, main_clock, main_window, wordle_game):
    wordle_game.game_count += 1
    actual_word = solution
...
#     print(len(manual_guess))
    ...
    while True:
        for event in pygame.event.get():
            ...
            if wordle_game.game_mode == PlayerMode.manual:
    ...
    #                 print(len(guess))
                    ...
                    if event.key == K_RETURN and win == True:
                        manual(SCREEN_WIDTH, SCREEN_HEIGHT, FPS, clock, window, wordle_game)
                    ...
                    if event.key == K_RETURN and len(manual_guess) > 4:
                        win = wordle_game.check_guess(turns, actual_word, manual_guess, window)
                        turns += 1
            ...
            if wordle_game.game_mode == PlayerMode.automatic:
                for event in pygame.event.get():
                    if event.type == QUIT:
                        pygame.quit()
                        sys.exit()
        ...
        #user has typed in a letter/pressed key
            if event.type == KEYDOWN:
                ...
                if event.key == pygame.K_m:
                    ...
                    wordle_game = WordleGame(GameDifficulty.normal, PlayerMode.manual)
                    manual(SCREEN_WIDTH, SCREEN_HEIGHT, FPS, clock, window, wordle_game, actual_word)
